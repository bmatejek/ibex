...
Over and under-segmentation are quantified by the conditional entropy $H(GT | SG)$ and $H(SG | GT)$, respectively. Since we are measuring the entropy between two clusterings, better split variation of information scores are closer to the origin. The variation of information values of the original segmentation approaches at different stopping thresholds of agglomeration for both algorithms are shown in Fig.~\ref{fig:variation-of-information}. \hp{check}
