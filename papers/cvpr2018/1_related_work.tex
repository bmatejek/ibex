...
Many methods build on top of these methods and train random-forest classifiers \hp{really?} to produce region-based (i.e., super-pixel) segmentations~\cite{seymour2016rhoananet,nunez2014graph,10.1371/journal.pone.0125825,parag2017anisotropic,zlateski2015image}. \hp{are these agglomeration techniques based on watershed?}
...
Even though graph partitioning is an NP-Hard problem~\cite{demaine2006correlation} there are several useful multicut heuristics that provide good approximations with reasonable computational costs~\cite{horvnakova2017analysis,keuper2015efficient}. We use the method of X et al.\hp{add ref} to partition the extracted 3D graph into the final neuron reconstruction.
