...
We use one of these segmentations (green circle) as out input dataset with an agglomeration threshold of 0.3 for all datasets. 
...
This is because the baseline segmentation algorithm for the isotropic FlyEM data (Sec.~\ref{sec:neuroproof}) performs much better, reducing the potential for improvements. 
...
Fig.~\ref{fig:qualitative-results} shows successful merges on the Kasthuri Vol. 2 dataset. 
...
Fig.~\ref{fig:qualitative-results} shows typical failure cases of our method (red circles).
