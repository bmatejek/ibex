...
\TODO{[refer to teaser figure of the pipeline]}
\TODO{[need to be concise here]}
\TODO{[we use biological constraints]}
...
Most agglomeration methods create the region graph by removing small-sized segments and linking each pair of adjacent segments, which can still lead to a large graph size due to the irregular shape of neural structures.
We here use the skeleton representation of the segmenation and reduce the graph size with the geometric constraints on the connectivity of two adjacent segments to prune edges.
...
Our key observation is that if two segments belong to the same neural structure, their skeleton end points should satisfy certain geomtry constraints.
...
\TODO{maybe good to have equation, to make the biological constraints stand out more}
Two segments, $s_1$ and $s_2$, receive a corresponding edge if the two following conditions hold. \TODO{what's the intuition}
...
We aim to assign edge weights $w_e$ to each edge corresponding to the possibility of two nodes belonging to the same neural structure.
W, we train a 3D CNN model to learn these geometric contraints for valid connection between two segments from labeled volume.

\noindent\textbf{Edge Probability}
As it's hard to manually design the weight
Below, we describe the network we trained to predict $p_e$.

%\subsubsection{Network Architecture}
Our network is a feedforward convolutional network with three \textit{VGG-style} convolution blocks~\cite{chatfield2014return} and two fully connected layers before the final sigmoid activation. 
Each \textit{VGG-style} convolution block consists of two double convolutions with filter size $3\times3\times3$ and LeakyReLU(0.0005), followed by a max pooling with filter size \TODO{xx}. 
...
To add more control.. \TODO{what's the intuition}
...
intro: blah,blah. \TODO{can we have a formula here?}
