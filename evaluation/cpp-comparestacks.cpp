...
void CppGetStatsFromVolume(EvaluationStats *output, long *segmentation, long *gold, long grid_size[3], long *ground_truth_masks, long nmasks) {
    ...
    for (long iv = 0; iv < nentries; ++iv) {
        if (masked_gold_labels.find(gold[iv]) == masked_gold_labels.end()) output->nnonzero++;
    }
    // create mappings from ij, i, and j to number of elements
    for (long iv = 0; iv < nentries; ++iv) {
        if (masked_gold_labels.find(gold[iv]) != masked_gold_labels.end()) continue;

        output->c[gold[iv]][segmentation[iv]]++;
        output->s[segmentation[iv]]++;
        output->t[gold[iv]]++;
    }
}
...
double* CppEvaluateStat(EvaluationStats *stat) {
    ...
    for (auto& a: stat->c)
        for (auto& b: a.second)
            TP += (NChoose2(b.second));
    long TP_FP = 0;
    for (auto& a: stat->s)
        TP_FP += NChoose2(a.second);
    long TP_FN = 0;
    for (auto& a: stat->t)
        TP_FN += NChoose2(a.second);
    ...
    for (auto& a:stat->c) {
        ...
        for (auto& b: a.second) {
            ...
            double spi = stat->s[is] / (double)(stat->nnonzero);
            double tpj = stat->t[ig] / (double)(stat->nnonzero);
            double pij = b.second / (double)(stat->nnonzero);
    ...
    results[0] = FP / (double) (NChoose2(stat->nnonzero));
    results[1] = FN / (double) (NChoose2(stat->nnonzero));
    results[2] = VI_merge;
    results[3] = VI_split;
