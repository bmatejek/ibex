...
{
  printf("Add support for offsets like skeletonization does\n");
  exit(-1);
  ...
  while (current_index < nentries) {
    ...
    while (pixels.size()) {
      ...
      for (long iw = -1; iw <= 1; ++iw) {
        if (iz + iw < 0 or iz + iw > grid_size[IB_Z] - 1) continue;
        for (long iv = -1; iv <= 1; ++iv) {
          if (iy + iv < 0 or iy + iv > grid_size[IB_Y] - 1) continue;
          for (long iu = -1; iu <= 1; ++iu) {
            if (ix + iu < 0 or ix + iu > grid_size[IB_X] - 1) continue;
...
void CppDownsampleMapping(const char *prefix, long *segmentation, long input_resolution[3], long output_resolution[3], long input_grid_size[3], bool benchmark)
{
  ...
  long nentries = input_grid_size[IB_Z] * input_grid_size[IB_Y] * input_grid_size[IB_X];
